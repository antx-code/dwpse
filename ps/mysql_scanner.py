    ...
    def create_connect(self, *args):
        try:
            connection = create_engine(
                f"mysql.csv+pymysql://{args[2]}:{urlquote(args[3])}@{args[0]}:{args[1]}",
                encoding='utf-8', connect_args={'connect_timeout': self.timeout})
        except ConnectionError:
            connection = create_engine(
                f"mysql.csv+mysqlconnector://{args[2]}:{urlquote(args[3])}@{args[0]}:{args[1]}/mysql.csv?auth_plugin=mysql_native_password",
                encoding='utf-8', connect_args={'connect_timeout': self.timeout})
        except Exception as e:
            connection = False
        return connection
    ...
    def dia(self, mode: str, target_file: str, passwords: Union[list, str] = None,
        ...
        for ip_port in ips:
            ...
            for user_pwd in passwords:
                ...
                if result:
                    asset_io.save2file('mysql_success', ip, port, username, password)
